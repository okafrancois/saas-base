...
interface ReviewFormProps {
  data: {
    requestType?: Partial<RequestTypeFormData>
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 p-4">
          <InfoField
            label={t('request_type.type.label')}
            value={data.requestType?.documentType
              ? t(`request_type.type.${data.requestType.documentType.toLowerCase()}`)
              : undefined}
            isRequired
          />
          <InfoField
            label={t('request_type.nationality.label')}
            value={data.requestType?.nationalityAcquisition
              ? t(`request_type.nationality.${data.requestType.nationalityAcquisition.toLowerCase()}`)
              : undefined}
            isRequired
          />
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4 p-4">
          ...
          {data.contactInfo?.addressGabon ? (
            <>
              <Separator />
              <div>
                <span className="text-sm text-muted-foreground">{t('form.address_gabon')}</span>
                <p className="font-medium">{data.contactInfo.addressGabon.address}</p>
                <p className="font-medium">
                  {data.contactInfo.addressGabon.district}, {data.contactInfo.addressGabon.city}
                </p>
              </div>
            </>
          ) : (
